...
public class LhaArchiveInputStream extends ArchiveInputStream<LhaArchiveEntry> {
    ...
    protected LhaArchiveEntry readHeaderLevel0(ByteBuffer buffer) throws IOException {
        ...
        String compressionMethod = getCompressionMethod(buffer);

        LhaArchiveEntry.Builder entryBuilder = new LhaArchiveEntry.Builder()
        ...
        entryBuilder.setDirectory(isDirectory(compressionMethod));

        entryBuilder.setCrcValue(Short.toUnsignedInt(buffer.getShort()));

        if (calculateHeaderChecksum(buffer) != headerChecksum) {
            throw new ArchiveException("Invalid header level 0 checksum");
        }

        LhaArchiveEntry entry = entryBuilder.get();
    ...
    protected LhaArchiveEntry readHeaderLevel1(ByteBuffer buffer) throws IOException {
        ...
        entryBuilder.setCrcValue(Short.toUnsignedInt(buffer.getShort()));
        entryBuilder.setOsId(Byte.toUnsignedInt(buffer.get()));
